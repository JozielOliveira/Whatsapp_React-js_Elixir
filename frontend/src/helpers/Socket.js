...
export class Channel {
  ...
  join(timeout = this.timeout){
    if(this.joinedOnce){
      throw(`tried to join multiple times. 'join' can only be called a single time per channel instance`)// eslint-disable-line  no-throw-literal
    } else {
  ...
  push(event, payload, timeout = this.timeout){
    if(!this.joinedOnce){
      throw(`tried to push '${event}' to '${this.topic}' before joining. Use channel.join() before pushing events`)// eslint-disable-line  no-throw-literal
    }
...
export class Socket {
  ...
  protocol(){ return document.location.protocol.match(/^https/) ? "wss" : "ws" }
  ...
  endPointURL(){
    ...
    return `${this.protocol()}://${document.location.host}${uri}`
  }
  ...
  onConnMessage(rawMessage){
    ...
    this.log("receive", `${payload.status || ""} ${topic} ${event} ${ref && "(" + ref + ")" || ""}`, payload)// eslint-disable-line   no-mixed-operators
...
export class LongPoll {
  ...
  normalizeEndpoint(endPoint){
    return(endPoint
      .replace("ws://", "http://")
      .replace("wss://", "https://")
      .replace(new RegExp("(.*)\/" + TRANSPORTS.websocket), "$1/" + TRANSPORTS.longpoll))// eslint-disable-line  no-useless-escape
  }
    ...
    Ajax.request("GET", this.endpointURL(), "application/json", null, this.timeout, this.ontimeout.bind(this), (resp) => {
      ...
      switch(status){
        ...
        case 500:
          ...
          this.closeAndRetry()
          break
        default: throw(`unhandled poll status ${status}`) // eslint-disable-line no-throw-literal
      }
    })
  }
